...
public class QuartzJob {
	...
	public void doStatisticOneHour() {
		...
		String sqlInsert = "insert into locationtemp(x,y,z,idType,dataType,timestamp,userID)"
				...
				+ "FROM " + tableName + " where FROM_UNIXTIME(timestamp/1000,'%Y%m%d%H%i00') = " + time
	...
	public void doStatisticAreaByDay() {
		...
		try {
			for (AreaModel l : ResultList) {
				areaName = l.getId();
				areaNumber = String.valueOf(linemapDao.getAreaNumberByDay(areaName));
	...
	public void doStatisticVisitTime() {
		...
		String visitDay = ConvertUtil.dateFormat(currentDate.getTime(), "yyyy-MM-dd");
		...
		url = dbs[0] + sva + '?' + "user=" + name + "&password=" + password + '&' + db1;
		...
		try {
			...
			for (AreaModel l : ResultList) {
				...
				cs = conn.prepareCall(
						"{CALL areaData('" + allTime + "'," + numbers + ',' + time + ',' + floorNo + ",'" + tableName
								+ "'," + x + ',' + x1 + ',' + y + ',' + y1 + ',' + areaId + ",'" + visitDay + "')}");
				cs.execute();
			}
	...
	public void doMinuteAllPeople() {
		...
		currentDate1.set(Calendar.MINUTE, 0);
		currentDate1.set(Calendar.SECOND, 0);
		String yesTableName = Params.LOCATION + ConvertUtil.dateFormat(currentDate1.getTime(), Params.YYYYMMDD);
		String nowTime = ConvertUtil.dateFormat(currentDate.getTime(), "yyyy-MM-dd HH:mm:ss");
		String yesterDay = ConvertUtil.dateFormat(currentDate1.getTime(), "yyyy-MM-dd");
		String visitDay = ConvertUtil.dateFormat(currentDate.getTime(), "yyyy-MM-dd");
		...
		for (StoreModel store : stores) {
			...
			placeId = store.getId();
			nowNumber = daoMaps.getNowPeople(tableName, String.valueOf(placeId), String.valueOf(time));
			number = daoMaps.getNumberByMinute(tableName, String.valueOf(placeId));
			yesNumber = daoMaps.getYesterdayNumber(yesTableName, String.valueOf(placeId), yesTimes);
...
//		List<Map<String, Object>> outList = null;
		...
		AreaInputModel inputModel = null;
		AreaInputModel updateModel = null;
//		long outTime = 0;
//		long oldTimestamp = 0;
		...
		for (int i = 0; i < allPeople.size(); i++) {
			...
			for (int j = 0; j < allArea.size(); j++) {
				...
				log.info("x:" + x + " y:" + y + " x0:" + x0 + " y0:" + y0 + " x1:" + x1 + " y1:" + y1 + " areaId:"
						+ disId + " floorNO:" + floorNo + " z:" + z);
				// 判断该人是否在该区域内
...
//						outList = dao.doTest1(sqlChk);
						int result = dao.doTest(sqlChk);
						log.info(result);
						if (result == 0) {
							inputModel = new AreaInputModel();
							...
							inputModel.setLocaltime(time_local);
							inputList.add(inputModel);
							log.info("insert:start");
//							String sqlIns = "insert into district_during(idType,timestamp,time_begin,"
//									+ "time_local,during,dataType,district_id,userID,loc_count) values ('" + idType
//									+ "'," + timestamp + "," + timestamp + "," + time_local + ",0,'" + dataType + "',"
//									+ disId + ",'" + userID + "',1)";
//							dao.doUpdate(sqlIns);
						} else {
//							outTime = (long) outList.get(0).get("departure_time");
//							oldTimestamp = (long) outList.get(0).get("timestamp");
							updateModel = new AreaInputModel();
							...
							updateModel.setLocaltime(time_local);
							updateList.add(updateModel);
							// String sqlStime = "select
							// departure_time,timestamp,time_local from
							// district_during "
							...
							// if (lastTime + visitTime * 60 * 1000 <
							// time_local)
							// {
							// departure_time = departure_time + time_local
							// - lastTime;
							// }
							// String sqlUpd = "update district_during set
							// departure_time=?, timestamp=?, time_local=?"
							// + " where district_id=? and userID=?";
//							String sqlUpd = "update district_during set timestamp=?"
//									+ " where district_id=? and userID=?";
//							dao.updateDistrict1(sqlUpd, timestamp, disId, userID);
						}
					} catch (Exception e) {
						log.info(e.getStackTrace());
					}
		...
		int areaId = 0;
		int during = 0;
		AreaInputModel model = null;
		AreaInputModel model1 = null;
...
//		System.out.println(sqlup);
		System.out.println("insertSiz:"+inputList.size()+" updateSiz:"+updateList.size());
		try {
			if (updateSize>0) {
				dao.doUpdate(sqlup);
			}
	...
	public void saveVisitiTime() {
		...
		long size = 0;// 人数
		String visitDay = ConvertUtil.dateFormat(currentDate.getTime(), "yyyy-MM-dd");// 当前日期
		...
		for (int i = 0; i < allArea.size(); i++) {
			...
			if (lis.size() > 0) {
				for (int j = 0; j < size; j++) {
					...
					beginTime = Long.valueOf(lis.get(j).get("time_begin").toString());
					LastTime = Long.valueOf(lis.get(j).get("timestamp").toString());
					likaiTime = Long.valueOf(lis.get(j).get("departure_time").toString());
			...
			String insetSql = "REPLACE INTO staticvisit(areaId,time,allTime,number,averageTime) VALUES(" + areaId + ",'"
					+ visitDay + "'," + visiTime + "," + size + "," + visiTime + ")";
